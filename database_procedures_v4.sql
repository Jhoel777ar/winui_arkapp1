...
-- 1. SP Modificar Compra (Versión Definitiva con Trim y Actualización)
...
BEGIN
    ...
    BEGIN TRY
        ...
        -- C. Procesar JSON de nuevos productos (CON TRIM)
        ...
        SELECT
            LTRIM(RTRIM(Codigo)), -- IMPORTANTE: Trim para evitar duplicados "PROD " vs "PROD"
        ...
        -- D. Identificar o Crear Productos
        -- D.1 Vincular existentes por CÓDIGO EXACTO
        UPDATE t
        SET t.ProductoExiste = 1, t.ProductoId = p.Id
        FROM @TempProductos t
        INNER JOIN Productos p ON t.Codigo = p.Codigo;

        -- D.2 ACTUALIZAR datos de productos existentes (Nombre, Precios, etc.)
        UPDATE p
        SET
            p.Nombre = t.Nombre,
        ...
        -- D.3 Insertar NUEVOS productos (solo si no existen)
        ...
        SELECT
            ...
            t.PrecioCompra, t.PrecioVenta, 0.00, -- Stock se suma luego
            t.UnidadMedida, t.StockMinimo, 1, GETDATE()
        FROM @TempProductos t
        WHERE t.ProductoExiste = 0;

        -- D.4 Vincular los recien insertados
        UPDATE t
...
-- 2. SP Registrar Compra (Versión Definitiva con Trim)
...
BEGIN
    ...
    BEGIN TRY
        ...
        SELECT
            t.Codigo, t.Nombre, t.CategoriaId, t.Talla, t.Color,
            t.PrecioCompra, t.PrecioVenta, 0.00,
            t.UnidadMedida, t.StockMinimo, 1, GETDATE()
        FROM @TempProductos t
        WHERE t.ProductoExiste = 0;
